...
# hso Function
# =============================================================================
def hso(fun, lb, ub, bw, *args, **kwargs):
	
	'''
	HSO function - Python Version of the Harmony Search Optimizer
	...
	# Define Inputs
	HMS = options['HMS']            # Memory Size [4,10]
	HMCR = options['HMCR']          # Probability rate of choosing from memory [?, ?]
	PAR = options['PAR']            # Pitch adjustment rate [0.3,0.99]
	maxIter = options['MaxIter']    # Maximum number of improvisations
	ND = len(ub)                    # Number of dimensions
	...
	while status != 1:
		...
		if (sum(gvals) <= 0):
			...
			if (fval == hmin):
				...
				print x
				print fval
				print numpy.var(numpy.corrcoef(HM).T)
				#if (options['Display'] == 'iter'):

     ...
     1  CON,SIM,SIMI,DATMAT,A,VSIG,VETA,SIGBAR,DX,W,IACT,IFAIL,NFVALS,
     2  IOUT,IFILE,FF,GG)
     ...
     1  A(N,*),VSIG(*),VETA(*),SIGBAR(*),DX(*),W(*),IACT(*),GG(*)
C
...
C     Fix compiler warnings
     ...
     1      STATUS='UNKNOWN')
	  END IF
      IF (IPRINT .GE. 2) WRITE(IOUT,10) RHO
...
C
   ...
   50      FORMAT (/3X,'Return from subroutine COBYLA because the ',
     1        'MAXFUN limit has been reached.')
      ...
      NFVALS=NFVALS+1
      IF (IPRINT .EQ. 3) THEN
         WRITE(IOUT,*) '  SIM = ', (SIM(J,NP),J=1,N)
         WRITE(IOUT,*) '  DX = ', (DX(I),I=1,N)
         WRITE(IOUT,*) '  BEFORE: ',N,M,(X(I),I=1,N),F,(CON(I),I=1,M)
      END IF
   ...
   65 GG(I)=CON(I)
      IF (IPRINT .EQ. 3) THEN
         WRITE(IOUT,*) '  AFTER: ',N,M,(X(I),I=1,N),F,(CON(I),I=1,M)
      END IF
      RESMAX=0.0d0
      IF (M .GT. 0) THEN
          DO 60 K=1,M
   60     RESMAX=DMAX1(RESMAX,-CON(K))
      ...
      IF (NFVALS .EQ. IPRINT-1 .OR. IPRINT .EQ. 3) THEN
          WRITE(IOUT,70) NFVALS,F,RESMAX,(X(I),I=1,IPTEM)
   70     FORMAT (/3X,'NFVALS =',I8,3X,'F =',1PE13.6,4X,'MAXCV =',
     1      1PE13.6/3X,'X =',1PE13.6,1P4E15.6)
          IF (IPTEM .LT. N) WRITE(IOUT,80) (X(I),I=IPTEMP,N)
...
C
  ...
  260 CONTINUE
      IF (IPRINT .EQ. 3) THEN
         WRITE(IOUT,*) '  SIMI = ', ((SIMI(I,J),I=1,N),J=1,N)
         WRITE(IOUT,*) '   SIM = ', ((SIM(I,J),I=1,N),J=1,N)
         WRITE(IOUT,*) '  VSIG = ', (VSIG(J),J=1,N), ' -- ', PARSIG
         WRITE(IOUT,*) '  VETA = ', (VETA(J),J=1,N), ' -- ', PARETA
         WRITE(IOUT,*) '  IBRNCH, IFLAG = ', IBRNCH, IFLAG
         WRITE(IOUT,*) '  A = ', ((A(I,J),I=1,N),J=1,MP)
      END IF
C
...
C
      ...
      IF (PARMU .LT. 1.5d0*BARMU) THEN
          PARMU=2.0d0*BARMU
          IF (IPRINT .GE. 2) WRITE(IOUT,410) PARMU
...
C
  ...
  570         CONTINUE
              ...
              END IF
          END IF
          IF (IPRINT .GE. 2) WRITE(IOUT,580) RHO,PARMU
  580     FORMAT (/3X,'Reduction in RHO to',1PE13.6,'  and PARMU =',
     1      1PE13.6)
          IF (IPRINT .EQ. 2) THEN
              WRITE(IOUT,70) NFVALS,DATMAT(MP,NP),DATMAT(MPP,NP),
